...
int execution_logic(t_node *ast, t_minishell *minishell)
{
    ...
    else if (ast->type == REDIRECTION)
    {
        ...
        return execution_logic(ast->child, minishell); // Execute the child command
...
Best Practices:
Explicit Ownership:
Decide who is responsible for freeing memory (e.g., parser frees tokens after AST creation).
Freeing AST Nodes:
...
void execute_ast(Node *node) {
    ...
    switch (node->type) {
        case NODE_COMMAND:
            execute_command(node->data.command.command, node->data.command.args);
            break;
        case NODE_PIPE:
            execute_pipe_ast(node->data.pipe.left, node->data.pipe.right);
            break;
        case NODE_REDIRECTION:
            execute_with_redirection(node->data.redirection.child, node->data.redirection.filename, node->data.redirection.append);
            break;
        default:
            fprintf(stderr, "Unknown node type\n");
    }
...
t_node	*traverse_tree(t_node *ast, t_minishell *minishell, t_node *parent_node)
{
	if (!ast || minishell->minishell_err)
		return (ast); // return some sort of error (or exit?), depending how bash behaves
	if (node->type == T_PIPE) // if (node-type == T_AND || node->type == T_OR || node->type == T_PIPE)
	{
		if (ast->type == T_PIPE)
