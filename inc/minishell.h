...
t_token							*handle_redirection_expansion(t_token *token,
									t_minishell *shell);
t_token							*expand_all_tokens(t_token *tokens,
									t_minishell *shell);
...
void							link_tokens(t_token **head, t_token **current,
									t_token *new_token);
int								process_char(char c, int *in_squote,
									int *in_dquote, char **result);

void							append_char(char **result, char *ch);
void							append_str(char **result, char *str);
void							process_expansion_char(char *str, int *i,
									char **result, t_exp_context ctx);
void							handle_single_quote(int *i, char **result,
									int *in_squote);
void							handle_double_quote(int *i, char **result,
									int *in_dquote);
void							handle_variable_expansion(char *str, int *i,
...
t_ast							*parse_redirection(t_token **tokens,
									t_ast *left, t_minishell *shell);
size_t							count_arguments(t_token *tokens);
t_ast							*parse_heredoc(t_token **tokens, t_ast *left,
									t_minishell *shell);
...
void							init_fields(t_minishell *minishell,
									char **envp);
void							free_minishell(t_minishell *minishell);
t_env							*new_env_node(const char *key,
									const char *value);
...
int								update_pwds(t_minishell *shell,
									t_env **env_list, char *new_path);
int								change_directory(t_minishell *shell,
									t_env **env_list, char *path);
char							*expand_path(t_minishell *shell,
									t_env **env_list, char *path);
int								b_cd(t_minishell *shell, t_env **env_list,
									char **args);
void							print_args(char **args, int nflag);
int								b_echo(char **args);
int								b_env(t_env *env_list);
int								is_valid_exit_arg(char *arg);
void							cleanup_and_exit(t_minishell *shell,
									int exit_code);
...
int								have_equals(t_env **env, char *arg,
									char *equal_sign);
// signals
...
int								get_redirection_info(t_ast *node, int *flags,
									int *original_fd);
pid_t							exec_left(t_ast *node, t_minishell *shell,
									int input_fd, int pipe_fd[2]);
pid_t							exec_right(t_ast *node, t_minishell *shell,
									int pipe_fd[2]);
...
int								parse_and_exec(t_minishell *shell,
									int *exit_status);
